...
import {
  Loader2, ShieldCheck, Briefcase, CheckCircle2, Clock, Users,
  Link as LinkIcon, BarChart3, CalendarDays,
} from "lucide-react";
...
interface MemberTeam { team_id: string; team_name: string; position: string; team_logo_url?: string | null; }
interface MemberTask { id: string; title: string; status: string; review_note?: string | null; created_at: string; }
...
export function MemberProfileDialog({ userId, onClose }: Props) {
  ...
  useEffect(() => {
    if (!userId) { setProfile(null); setRole(null); setTeams([]); setTasks([]); return; }
    setLoading(true);

    Promise.all([
      supabase.from("profiles")
        .select("id, display_name, email, avatar_url, bio, github_url, instagram_url, facebook_url, internship_start, internship_end, discord_name")
        .eq("id", userId).single(),
    ...
    ]).then(async ([{ data: prof }, { data: roleData }, { data: memberships }, { data: taskData }]) => {
      setProfile((prof as any as MemberProfile) ?? null);
      setRole((roleData as any)?.role ?? null);
      setTasks((taskData as any as MemberTask[]) ?? []);
      ...
      if (memberships && (memberships as any[]).length > 0) {
        ...
        setTeams((memberships as any[]).map((m: any) => ({
          team_id: m.team_id,
  ...
  const name     = profile?.display_name || profile?.email?.split("@")[0] || "ไม่ทราบชื่อ";
  const initText = name.split(" ").map((w) => w[0]).join("").slice(0, 2).toUpperCase();
            ...
            <div className="overflow-y-auto flex-1 px-6 pb-4">
              {loading ? (
                ...
                <div className="space-y-5 py-4">
                  ...
                  <div className="border-t pt-4 space-y-3">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
                      <Users className="w-3.5 h-3.5" /> ทีมที่สังกัด ({teams.length} ทีม)
                    </p>
                    {teams.length === 0 ? (
